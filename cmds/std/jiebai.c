...
int main(object me, string arg)
{
	...
	if( userp(obj) && (object)obj->query_temp("pending/jiebai")!=me ) {
		message_vision("\n$N對着$n説道：" + RANK_D->query_self(me) + me->name() + "，對" + RANK_D->query_respect(obj)+ "心儀已久，願彼此生死與共，肝膽相照！\n\n", me, obj);
		...
		tell_object(obj, YEL "如果你願意和對方結為異姓骨肉，請你也對" + me->name() + "("+(string)me->query("id")+")"+ "下一次 jiebai 指令。\n" NOR);
		write(RED "對方必須同意才能結拜。你等着吧。\n" NOR);
		return 1;
	}

	message_vision("\n$N對着$n高聲説道：不求同年同月同日生，但願同年同月同日死！\n\n", me, obj);

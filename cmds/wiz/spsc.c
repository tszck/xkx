...
int main(object me, string arg)
{
	...
	else
	{
		...
		if(!wizardp(me))
		{
			if(!ob) return notify_fail("只有察看你的狀態。\n");
			if(!ob->query("parents"))
				return notify_fail("巫師才能察看別人狀態。\n");
			else
				if(member_array(me->query("id"),ob->query("parents"))<=-1)
					return notify_fail("只有爹孃能察看寶寶的狀態。\n");
		}
		else if (!ob)
			return notify_fail("此人不在線，請用sc查看數據。\n");
	}
	...
	if (userp(ob))
		line += sprintf(" 你是一%s%s歲%s個月性格%s的%s%s，%s生。\n",
		ob->query("unit"),
		...
		line += sprintf(" 你是一%s%s歲性格%s的%s%s，%s生。\n",
		ob->query("unit"),
	...
	{
		line += sprintf(
	" 靈性[%d/%s]  福緣[%d/%s]  膽識[%d/%s]  定力[%d/%s]  容貌[%d/%s]\n",
	...
	if (userp(ob))
	{
		...
		} else line = sprintf("%s 你還沒有師父。", line );
		...
		} else line += " 你的伴侶是 "+ob->query("couple/couple_name")+"。\n";

...
void create()
{
	set_name("王教頭", ({ "wang jiaotou", "wang","jiaotou"}));
...
int accept_object(object who, object ob)
{
    ...
    {
		message_vision("王教頭望着$N説：你的武功應該歷練江湖才能再長進，不能埋沒
在這裏了。\n", who);
		return 0;
    }
    if (ob->query("money_id") && ob->value() >= 500)
    {
		...
		message_vision("王教頭對$N説：好！這位" + RANK_D->query_respect(who) 
+ "想學什麼呢？\n" , who);
		return 1;
    }
    else
		message_vision("王教頭皺眉對$N説：錢我不在乎。可你也給的也太少了點兒吧？\n", who);
	return 0;
}

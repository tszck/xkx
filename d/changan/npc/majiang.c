...
int do_sit()
{
	...
	||  me->query("id")==P4Data["Id"])
	{
		...
		else message_vision("$N對着大家說：今天自摸"+(random(12)+1)+"次就好。\n",me);
		return 1;
	}
	if (Playing)
	{
		if (random(1))
			...
			message_vision("$N哭哭啼啼不停地說道：我也要玩啦～～。\n",me);
		return 1;
	}
...
int do_da(string str,object me)
{
	...
	if (Goned==5)
	{
		message_vision("$N說道：此局因"+HIR+"五花合牌"+NOR+"請再重新開始(start)。\n",this_object());
		Playing=0;
		Count++;
		if (Play==4)
		{
...
int do_eat(string str)
{
	...
	if (LastDumpMan==0)
		return notify_fail("你沒"+do_check_num(num1,0)+"這張牌。\n");
	LastDumpMan=do_check_Mj(who["Mj"],num2);
	if (LastDumpMan==0)
		return notify_fail("你沒"+do_check_num(num2,0)+"這張牌。\n");
...
int do_gang(string str,object me)
{
	...
	if (i==1)
	{
		...
		if (NO_GANG) return notify_fail("這樣無法槓牌。\n");
		message_vision("\n$N說道："+HIB+"木工"+NOR+"～～～\n\n",me);
		who["Mj"]=do_delete_Mj(who["Mj"],str,1);
...
int do_setmj(string str)
{
	...
	if (sscanf(str,"%s %s",str,numstr)!=2) return notify_fail("指令:setmj <tin>&<gang>&<peng> <代碼>\n");
	if (str=="tin")
	{
		if (numstr=="none")
		{

...
string * inroad_msg = ({
	...
	"$N滿臉堆笑，對$n說道：“『光棍不喫眼前虧，伸手不打笑臉人。』在下到那邊方便方便。”\n",
});
...
int do_baobiao(string arg)
{
	...
	if (!me->query_temp("fuwei_ready"))
		return notify_fail(this_object()->query("name") +"說：“去去去，去帳房交錢去！”\n"); 
	if ( query("leader_name") != me->query("id") )
		return notify_fail(this_object()->query("name") +"說：“你沒有負責保護此鏢的義務！”\n"); 

...
void guojxjob(object ob,int maxexp,int time)
{
	...
	{
		message_vision(HIY "突然間從路旁閃出一隊蒙古韃子，隊伍中夾雜着一個蒙面人，行色匆匆，似乎急着要離開！\n"NOR, ob);
		message_vision(HIY "$N衝着蒙面人大喝一聲：大膽奸細，偷了襄陽城防圖，還想跑麼？\n"NOR, ob);
		message_vision(HIY "蒙面人一看事情敗露，狂笑不已：就憑你們幾個也想擋住我的去路？\n"NOR, ob);
		for(j=0;j<time;j++)
		{
		for(i=0;i<count;i++)
		{

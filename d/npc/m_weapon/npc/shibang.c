...
void create()
{
	...
	set("inquiry", ([
		"鑄棒" : "師傅命我幫助別人鑄棒，但到現在也不見一個人影。誰要鑄棒呢？\n",
		"原料" : "鑄棒可用寒絲羽竹，海底金母，千年神木，隕星鐵石。但效果各不一樣。\n",
	]) );
	setup();
...
{	
	...
	{
		message_vision(HIC"師傅沒有同意為你鑄棒，您請回吧！\n"NOR, who);
		return 0;
	}
	...
	message("vision", "侍棒低頭檢視了一下手中的"+(string)ob->query("name")+"點了點頭説：\n", who);
	message("vision","。。。好我們現在就開始鑄棒\n", who);
	write("請這位"+RANK_D->query_respect(who)+"給棒起名字吧！(zhu 棒的名字 英文代號)\n");
	destruct(ob);
	return 1;
}
#include "shi.c";

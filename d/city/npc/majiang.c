...
int do_continue(object me);	//防止遊戲不能進行！具體爲什麼有時程序會停下來出錯原因荒涼不明白，估計是函數調用太深的緣故
...
"Attitude": "common", "Hu_type": "", "3Ps": "", "3Es":"", "2Ps": "", "2Es1": "", "2Es2": "", "1s": "",	//爲了能判斷出什麼牌而加入
"Pause": "", "H2Ps": "", "H2Es1": "", "H2Es2": "", "H1s": ""		//爲了判斷碰和喫及槓而加
]);
mapping P2Data = ([
...
int PAUSE = 0;			//判斷是否處於暫停狀態;PAUSE==1是爲NPC叫的暫停，任何玩家都可以用continue命令取消；PAUSE==2
				//是爲玩家叫的暫停，只有該玩纔可用continue命令解除
string destop="";		//是桌面喔。
		 ...
		 me->query("id")==P3Data["Id"] || me->query("id")==P4Data["Id"])
	{
		if (!Playing)
			message_vision("$N大喊一聲：我要打麻將！還有誰上？\n",me);
			 ...
			 message_vision("$N對着大家說：今天自摸"+chinese_number(random(12)+1)+"次就好。\n",me);
		return 1;
	}
	if (Playing){
		if (random(1))
		message_vision(this_object()->name()+
		"對着"+me->name()+"說道：牌局正在進行中請到旁邊坐一下...。\n",me);
		else  message_vision("$N對着大家說：“我也要玩啦～～”不停的哭哭啼啼.....。\n",me);
		return 1;
	}
...
//	tell_object(me,"換誰打牌:"+NowPlayerId[NowPlayer]+"\n");
	...
	destop+=str;				//把牌加到桌面上，當然是爲了計算出什麼牌咯
	LastDump=str;
...
int do_eat(string str, object me)
{
	...
	message_vision("\n$N說道："+do_check_num(LastDump,0)+"～我～"+HIW+"口乞～～～"+NOR+"\n",me);
 	message_vision("$N拿出了一張"+HIG+do_check_num(num1,0)+NOR+
		      ...
		      "喫下了"+HIR+do_check_num(LastDump,0)+NOR+"。\n",me);
	...
	if( !userp(me) )
		call_out("dump_what", DELAY, me ); //如果喫牌者是電腦，調用函數計算出什麼牌。
...
varargs int do_win(string str,object me)
{
	...
	if (Touched&&NowPlayerId[NowPlayer]==me->query("id"))
	   message_vision("$N說道：胡家"+me->name()+HIG+"自摸"+NOR+"～～～～～～～"+HIR+"喫紅"+NOR+"～～～～～\n",this_object());
...
int do_setmj(string str)
{
	...
	if (!str){
	   numstr=this_object()->name()+"對着你說你設定的有：\n";
	  ...
	  if (who["Show"]=="代碼")    numstr+="顯示方式爲"+HIY+"[代碼]"+NOR+"方式。\n";
	  if (who["Show"]=="color")   numstr+="顯示方式爲"+HIG+"[彩色]"+NOR+"。\n";
...
//把英文字轉爲代碼 or 中文 flag 1 就是轉爲代碼 flag 2 轉爲1..9
mixed do_check_num(string str,int flag)
{
	int number;
...
int do_check_Mj(string datastr,string str)
{
	...
	for(i=0; i<strwidth(datastr); i+=2)
 ...
 string Mj="",p1o="",p2o="",p3o="",p4o="";
	...
	if (arg=="1p"||arg=="2p"||arg=="3p"||arg=="4p"){
	   ...
	   Mj+="你的設定有：";
	      ...
	      if (who["Show"]=="代碼") i=3;
	      if (who["Show"]=="color") i=5;
	      tell_object(me, "現在是["+TIMES+"] 莊家 "+HIY+present(NowPlayerId[0])->query("name")
	      	+"("+NowPlayerId[0]+")"+NOR+"\n" );
	      Mj+=HIG+"東桌  "+NOR+present(P1Data["Id"])->query("name")+"("+P1Data["Id"]+")"+"：擺在外面的牌有：\n";
	      Mj+=show_mj(next_data(P1Data["Id"], 0, "allout"),i+1);
	      Mj+=HIG+"南桌  "+NOR+present(P2Data["Id"])->query("name")+"("+P2Data["Id"]+")"+"：擺在外面的牌有：\n";
	      Mj+=show_mj(next_data(P2Data["Id"], 0, "allout"),i+1);
	      Mj+=HIG+"西桌  "+NOR+present(P3Data["Id"])->query("name")+"("+P3Data["Id"]+")"+"：擺在外面的牌有：\n";
	      Mj+=show_mj(next_data(P3Data["Id"], 0, "allout"),i+1);
	      Mj+=HIG+"北桌  "+NOR+present(P4Data["Id"])->query("name")+"("+P4Data["Id"]+")"+"：擺在外面的牌有：\n";
	...
	if (arg=="mj"){
	  ...
	  if (who["Show"]=="代碼")    Mj+="顯示方式爲代碼方式。";
	  if (who["Show"]=="color")   Mj+="顯示方式爲彩色。";
...
int get_tc(mapping who)
{
	...
	if (check!=0&&tempB!=""&&(check+strwidth(tempB)/2>14)) Tc+="@I";//混幺九
...
//以下是荒涼加入的函數爲，一方面爲了能人機對局而加，另外也通過徵詢玩家們的意見加入了一些人性化的函數命令……
//從這裏開始是人性化的函數
...
int undo_pause(object me)
{
	...
	tell_room( environment(this_object()), this_object()->query("name")+"說道：“大家儘量緊湊一些。\n" );
...
int npc_action( object me )
{
 ...
 int i;
	...
	//}
		...
		//NPC暫時只會槓寶牌，因爲槓其它牌容易斷了胡路……
		//tell_object(present("wiman"), "槓牌 ");
		if( !userp(ob) ) {
			str = who["3Ps"];
			if( do_check_Mj(str, LastDump)==3 && do_check_num(LastDump, 1)>27 && strwidth(who["Mj"])<26){
				//不立直就槓
				//PAUSE = 1;
	...
	//str = "";
		...
		i = sizeof(str);
		//tell_object(present("wiman"), me->query("name")+"喫牌1 ");
	...
	if( userp(me) ) {
		...
		{
			tell_object( me, "你可以用"+HIR+"chi"+NOR+"來喫牌！！\n");
			tell_object(me, "以下是你的牌：\n" +show_mj( str, 6) );
		}
	//		return 1;
	}
...
int do_continue(object me)
{
	...
	//if(!Touched) {
		...
		if( who["Pause"]=="yes" ) {
			...
			else tell_room( environment(me), me->query("id")+"對"+ob->query("id")+"說道：快摸牌啦！！\n");
		}
		if( me==present(NowPlayerId[NowPlayer])&&PAUSE!=2 ) {
			PAUSE =0;
			if(!Touched) do_touch(0, me);
			else tell_room( environment(me), me->query("id")+"說道：別急別急！\n");
		}
		else tell_room( environment(me), me->query("id")+"對"+ob->query("id")+"說道：快摸牌啦！！\n");
	//}
	//else {
...
int dump_samecolor( string str_out, string str_mj, object me)//, mapping Temps )
{
	...
	if( strwidth(Temps["tempT"]+Temps["tempB"]+Temps["tempS"])==0 || str_out=="" ) {
		size = strwidth(Temps["tempW"]);
	...
	if( strwidth(Temps["tempW"]+Temps["tempB"]+Temps["tempS"])==0 || str_out=="") {
		size = strwidth(Temps["tempT"]);
		get_all_temps( Temps, str_mj);
	...
	if(strwidth(Temps["tempW"]+Temps["tempB"]+Temps["tempT"])==0 || str_out=="") {
		size = strwidth(Temps["tempS"]);
		get_all_temps( Temps, str_mj);
	...
	if(strwidth(Temps["tempW"]+Temps["tempT"]+Temps["tempS"])==0 || str_out=="") {
		size = strwidth(Temps["tempB"]);
		get_all_temps( Temps, str_mj);
...
int dump_twocolor( string str_out, string str_mj, object me, mapping Temps )
{
	...
	if( Temps["tempT"]==""&&Temps["tempS"]=="" ) {
		size = strwidth(Temps["tempW"]+Temps["tempB"]);
		get_all_temps( Temps, str_mj);
	...
	if( Temps["tempW"]==""&&Temps["tempS"]=="" ) {
		size = strwidth(Temps["tempT"]+Temps["tempB"]);
		get_all_temps( Temps, str_mj);
	...
	if(Temps["tempW"]==""&&Temps["tempT"]=="") {
		size = strwidth(Temps["tempS"]+Temps["tempB"]);
		get_all_temps( Temps, str_mj);

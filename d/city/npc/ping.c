...
void greeting(object ob)
{
 int eff_qi, max_qi, hurt;
	...
	switch(hurt)
	{
		case 1 : message_vision("平一指滿臉關切地對$N説道：這位"+RANK_D->query_respect(ob)+"看來受了點輕傷，我這由上好的丹藥，不知是否要買些？\n",ob);
			break;
...
int recover(object ob)
{
	...
	message_vision("大約過了一盅茶的時份，平一指慢慢地站了起來。\n",ob);
...
int ask_over()
{
	...
	{
		message_vision("平一指驚訝的對$N説道：“哦！你再開什麼玩笑，我這可不是養老院啊！”\n",me);
		return 1;
	}

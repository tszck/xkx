...
string *tile_cname=({"天牌","地牌","人牌","合牌","梅花","長三","板凳","虎頭","屏風","七點","六點","雜九","雜八","雜七","雜五","至尊"}),
	*tile_face=({
...
void create()
{
	set("short", "牌九室");
	set("long", @LONG
這裏是推牌九的地方，屋子中間擺着一張大臺子。臺子旁邊立着個
小牌子 (paizi)。這裏賭的是小牌九，每門兩張牌，一翻一瞪眼。賭場
是莊家，賭客只能壓閒家。
LONG );
	...
	set("item_desc", ([
		"paizi" : "賭場抽稅五釐，本賭場不備零散碎銀。\n牌九的規矩玩法介紹請 help pj_rules。\n押注命令是 bet <銀兩> <sky(天門)|up(上門)|down(下門)>，只收銀子，限制十至百兩。\n\n命令範例： bet 88 silver sky\n",
	]));
	set("no_fight", "1");
	set("no_steal", "1");
	set("no_sleep_room", "1");
...
{
	tell_object(this_player(), "賭場裏喧嚷吵鬧，你根本靜不下心來。\n");
	return 1;
}
int valid_leave(object me, string dir)
{
	if (me->query_temp("p9/player"))
		return notify_fail("你正賭着錢呢，等這一把開了再走吧。\n");
...
int do_bet(string arg)
{
	...
	if (type != wtype)
		return notify_fail("這裏只接受"+cwtype+"！\n");
	...
	if( wager < min || wager > max )
		return notify_fail("你的賭注不在限額之內！\n" + "這裏的限額是" + chinese_number(min) + "至" + chinese_number(max) + "兩"+cwtype+"。\n");
	if (area != "up" && area != "sky" && area != "down")
		return notify_fail("你要賭哪一門？\n");
	if (stage >1) return notify_fail("已經開賭了，等下把吧。\n");
	if (me->query_temp("p9/player"))
		return notify_fail("你不是已經下注了嗎？！等着開牌吧！\n");
	if (!stage)
	{
		tell_room(__FILE__, HIY"莊家高聲喊道：“上場不分大小，只喫銀子元寶！英雄好漢，越輸越笑，王八羔子，贏了便跑！”\n"NOR);
		tell_room(__FILE__, HIG"賭客跟着起鬨：“下注啊，下注啊，買得多贏得多啊～～～”\n"NOR);
		stage = 1;
	...
	tell_room(__FILE__, me->query("name") + "壓了" + chinese_number(wager) + "兩"+cwtype+"在"+cchar[area] +"。\n");
...
// display tiles for each hand
	...
	for (i=0; i<4; i++)
	{
		...
		switch(hand_status[i])
		{
			case 2: hand_cname[i]="一對"+tile_cname[tiles[i][j]/2];
				break;
			...
			default:
				if (hand_value[i] == 0)
					...
					hand_cname[i]="一副"+chinese_number(hand_value[i])+"點";
		}
		switch (i)
		{
			case 1: str2 = "上門得到了："+hand_cname[i]+"    ";
				break;
			case 2: str2+= "天門得到了："+hand_cname[i]+"    ";
				break;
			case 3: str2+= "下門得到了："+hand_cname[i];
				break;
			default: str1= "                        莊家得到了："+hand_cname[i]+"\n";
		}
		for (k =0; k< 9; k++)
		{
			str1 += "                        ";
...
// pay out
	if (sizeof(winner)<1) tell_room(__FILE__, "莊家通喫！！\n");
	else if (sizeof(winner) >2) tell_room(__FILE__, "莊家通賠！！\n");
	else
	{
		str = "莊家喫： ";str2 ="莊家收取了 ";
...
+ "贏回"+chinese_number(wager*195/100)+"兩"+cwtype+"。\n");
//takes 5% tax

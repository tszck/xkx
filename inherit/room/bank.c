...
int do_deposit(string arg)
{
   ...
   {
     return notify_fail("命令格式：deposit <數量> <貨幣單位>\n");
   }
   ...
   me->start_busy(3);
        message_vision(sprintf("$N拿出%s%s%s，存進了錢莊。\n",
...
int do_withdraw(string arg)
{
   ...
   me = this_player();
        ...
        if (!arg || sscanf(arg, "%d %s", amount, what) != 2)
        {
                return notify_fail("命令格式：withdraw <數量> <貨幣單位>\n");
        }
   if( this_player()->is_busy() )
       return notify_fail("你現在正忙着呢...。\n");
...
{        
   // returns a chinese string of `amount` of money
        ...
        if (amount)
                return output + chinese_number(amount) + "文銅板";
        return output;
}

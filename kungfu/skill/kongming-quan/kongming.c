  ...
  if (userp(me) && !wizardp(me) &&
   ...
   return notify_fail("你所使用的外功中沒有這種功能。\n");
	...
	if( (int)me->query_temp("kongming") ) 
		return notify_fail(HIG"你已經在運動「空明」了。\n"NOR);
	...
	if( (int)me->query_skill(fskill, 1) < 150 )
		return notify_fail("你的"+to_chinese(fskill)+"修爲不夠，不能領悟"+PNAME+"。\n");

	if( (int)me->query_skill(sskill, 1) < 150 )
		return notify_fail("你的"+to_chinese(sskill)+"的修爲不夠，不能領悟"+PNAME+"。\n");

	if( !me->query_temp("murong/xingyi") )
	{
		if( me->query_skill_mapped("parry") != "kongming-quan") 
			return notify_fail("你未在招架中激發空明拳，無法使用空明！\n");
	}

	if( (int)me->query("neili") < 1200 )
		return notify_fail("你的內力還不夠！\n");
	
	msg = HIY "$N使出空明拳的絕技進行招架，拳法虛虛實實，\n" NOR;
	...
	{	
		...
		msg += HIY "$n兇狠的招數毫無用武之地，招招象打在空氣中！\n" NOR;
// 功效在combatd.c中體現
	}
	else{
		msg += HIR "但$n"HIR"不顧$N"HIR"拳法虛實，攻勢潮湧如故！\n" NOR;
	}
	me->add("neili", -1000);
...
void remove_effect(object me, int i, int j)
{
	if ( (int)me->query_temp("kongming") ) 
	{
		...
		tell_object(me, HIY "你的空明運行完畢，將內力收回丹田。\n" NOR);
	}
}

...
void saying(object ob)
{
	...
	if ((string)ob->query("gender") == "男性")
		message_vision("\n黃蓉面現驚奇之色，打量了$N一眼，說到：“你是什麼人，竟敢亂闖我的閨房？\n再不滾出去，看我怎麼整你！”\n", ob);
	else
		message_vision("\n黃蓉看了看$N，說到：“好久沒生人來過了，不知這位姐姐可有我靖哥哥的消息？\n", ob);
}
...
int ask_jiuyin()
{
		...
		&& me->query("family/master_id") == "ouyang feng" )
	{
		if(me->query("combat_exp")>500000 && present("mask_jiuyin",me))
		{
			...
			message_vision(HIG "$N大爲興奮，忙凝神傾聽，內息早於不知不覺中按照黃蓉所解之法四處遊走......\n", me);
			message_vision("$N的神智漸漸迷糊起來，忽而抓耳撓腮，忽而側頭苦思，種種怪狀，令人不解。\n", me);
			message_vision("突然間，$N反手啪啪啪連打自己三個耳光，雙手撐地，倒立了起來，全身真氣逆遊亂走 \n"NOR, me);
			if (random(5) == 3)
			{
				tell_object(me,"過了半晌，你只覺得全身經脈運行方式大爲異常，略一提氣，發現陰維，陽維，陰嶠， \n");
...
/*	if (me->query_temp("match_game/lost")) {
		message_vision( "$n仰起頭不屑的對$N說道：你太笨了，我纔不跟你玩呢。\n", me, ob );
		return 1;
	}*/

        ...
        {        // 若有資料輸入
                if (sscanf(str, "%s %s", _new, old) != 2)
                        return _message(pl, "輸入格式錯誤。", "_message_done",
                                HIDE);
                me["keymap"][_new] = old;
                _message(pl, sprintf("好了，從此以後新定義鍵「%s」就有原功能鍵「%s」的功能了！", _new, old), "_message_done", HIDE);
        }
        ...
        for (i=me["iS_Row"]+me["iRow"]-2; i<sizeof(me["sText"]); i++)
        {
                ...
                {        // 找到指定字串了, 移動遊標到目的行
                        ...
                        if (me["iCol"]<me["iS_Col"] ||
                            me["iCol"]+strwidth(me["sSearch"])>me["iS_Col"]+79)
                                _refresh_screen(pl, me["iS_Row"], me["iE_Row"],
                                        me["iCol"]);
                        ...
                        {        // 需要替換字串
                                ...
                                str += (me["sText"][i][me["iCol"]+
                                        strwidth(me["sSearch"])-1..<1]);
                                me["sText"][i] = str;
                                // 若替換字串落在原畫面外，則重繪畫面
                                if (me["iCol"]<me["iS_Col"] ||
                                    me["iCol"]+strwidth(me["sReplace"])
                                    > me["iS_Col"]+79)
                                        _refresh_screen(pl, me["iS_Row"],
                                                me["iE_Row"], me["iCol"]);
                                // 將遊漂移到替換字串後面
                                me["iCol"] = me["iCol"]+strwidth(me["sReplace"])-
                                        me["iS_Col"]+1;
                        ...
                        // 將遊漂移到搜尋字串後面
                                ...
                                me["iCol"] = me["iCol"]+strwidth(me["sSearch"])-me["iS_Col"]+1;
                        _refresh_status(pl);
                        break;
                } // if found
        } // for each row

...
string get_host_add(string alias);
...
string start_message()
{
   return sprintf( "||MUDNAME:%s||NAME:%s||VERSION:%s||MUDLIB:%s||HOST:%s||HOSTADD:%s||PORT:%d"
     "||PORTUDP:%d||TCP:%s",
     INTERMUD_NAME, Mud_name(),
     MUDLIB_VERSION, MUDLIB_NAME, query_host_name(),MY_ADD,
...
string get_host_add(string name)
{
  name = htonn(name);

  if (name == mud_nname())
    return this_host["HOSTADD"];
  if (undefinedp(muds[name]))
    return 0;
  return muds[name]["HOSTADD"];
}
...
varargs int idx_request(function f)
{
   if (file_name(previous_object())[0..strwidth(AUX_PATH) - 1] != AUX_PATH)
     return 0;
